#!/usr/bin/env python3
"""Normalize enum values to lowercase including plan field."""
...
def main():
    ...
    for record in data:
        ...
        # Normalize plan field to lowercase
        if 'plan' in record and isinstance(record['plan'], str):
            old_val = record['plan']
            record['plan'] = normalize_value(record['plan'])
    ...
    print(f"  plan:              {changes['plan']} records updated")
    print()
    ...
    # Show unique plan values
    unique_plans = sorted(set(r.get('plan', '') for r in data if r.get('plan')))
    print("Unique plan values after normalization:")
    print(f"  {', '.join(unique_plans)}")
